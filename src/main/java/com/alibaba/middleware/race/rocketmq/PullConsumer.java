...
public class PullConsumer {
	...
	public static void main(String[] args) throws MQClientException {
		final DefaultMQPullConsumer consumer = new DefaultMQPullConsumer(
				"test");
		...
		try {
			...
			// 循环每一个队列
			for (MessageQueue mq : mqs) {
				System.out.println("Consume message from queue: " + mq
						+ " mqsize=" + mqs.size());
				//int cnter = 0;
							...
							if (prSize != 0) {
								for (MessageExt me : result.getMsgFoundList()) {
				                    ...
				                    PaymentMessage paymentMessage = 
				                    		RaceUtils.readKryoObject(PaymentMessage.class, body);
