...
public class TaobaoSpoutPull implements IRichSpout {
	private static Logger LOG = LoggerFactory.getLogger(TaobaoSpoutPull.class);
	...
	public void open(Map conf, TopologyContext context,
		...
		consumer = new DefaultMQPullConsumer("taobao");
		...
		try {
			...
			consumer.registerMessageQueueListener(RaceConfig.MqTaobaoTradeTopic, null);
						...
						if (result.getMsgFoundList() != null) {
							...
							if (prSize != 0) {
								for (MessageExt me : result.getMsgFoundList()) {
									...
									OrderMessage orderMessage = RaceUtils
											.readKryoObject(
													OrderMessage.class, body);
									_collector.emit(new Values(orderMessage.getCreateTime(),
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		declarer.declare(new Fields("createTime","orderID"));
	}
